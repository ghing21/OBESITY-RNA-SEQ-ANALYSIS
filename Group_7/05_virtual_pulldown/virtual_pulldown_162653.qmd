...
df <- read.csv("../results/03_pval_integration/augment162653-01.csv")
...
ggsave("vp_162653_clusters.png", path = "../figures/")
```
...
```{r}
clus1_res <- fora(BP_list, clus_1, background) |> write_csv("../results/clus1_res.csv")
clus1_res
```

```{r}
clus2_res <- fora(BP_list, clus_2, background) |> write_csv("../results/clus2_res.csv")
clus2_res
```

```{r}
clus3_res <- fora(BP_list, clus_3, background) |> write_csv("../results/clus3_res.csv")
clus3_res
```
