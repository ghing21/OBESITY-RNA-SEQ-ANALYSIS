...
iso_count_matrix <- loadIsoformCountMatrix("GSE125856")
...
```{r}
gene_meta <- read_excel("./data/GSE125856/metadata_GSE125856.xlsx") |> 
  mutate(state = str_replace(state, "-", "_"))
```

Select only the samples that are present in the metadata (subcutaneous adipose tissue)
...
```{r}
ggsave("./figures/GSE125856_PCA.png", plot = PCA)
```
...
```{r}
res <- results(dds,
contrast = c("condition","Obese","Lean"))
```
...
```{r}
ggsave("./figures/GSE125856_fsc_p1.png", plot = fsc_p1)

ggsave("./figures/GSE125856_fsc_p2.png", plot = fsc_p2)

ggsave("./figures/GSE125856_fsc_p3.png", plot = fsc_p3)

ggsave("./figures/GSE125856_fsc_p4.png", plot = fsc_p4)
```
...
```{r}
ggsave("./figures/GSE125856_volcano.png", plot = volcano_p)
```

## Save results

```{r}
saveRDS(res_df, "./results/GSE125856_results.rds")
write_csv(res_df, "./results/GSE125856_results.csv")
```
