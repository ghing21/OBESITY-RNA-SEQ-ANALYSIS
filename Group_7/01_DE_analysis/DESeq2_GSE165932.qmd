...
iso_count_matrix <- loadIsoformCountMatrix("GSE165932")
...
```{r}
gene_meta <- read_excel("/net/pupil1/home/people/s243357/projects/introduction_to_systems_biology/sysbio_project/data/GSE165932/metadata_GSE165932.xlsx")
```
...
colData = gene_meta,
design = ~ insulin + condition)
```

## PCA for EDA

```{r}
PCA <- plotPCA(rlog(dds), intgroup=c("condition", "insulin"))
PCA
```

```{r}
ggsave("./figures/GSE165932_PCA.png", plot = PCA)
```

The same as in the other dataset, no clear separation between obese and control patients. Heterogeneous groups or maybe there is not a big differential expression.
...
```{r}
res <- results(dds,
contrast = c("condition","Obese","Normal"))
...
Many of them related with metabolism, some with immune activation
...
```{r}
ggsave("./figures/GSE165932_fsc_p1.png", plot = fsc_p1)

ggsave("./figures/GSE165932_fsc_p2.png", plot = fsc_p2)

ggsave("./figures/GSE165932_fsc_p3.png", plot = fsc_p3)

ggsave("./figures/GSE165932_fsc_p4.png", plot = fsc_p4)
```
...
```{r}
ggsave("./figures/GSE165932_volcano.png", plot = volcano_p)
```

## Save results

```{r}
saveRDS(res_df, "./results/GSE165932_results.rds")
write_csv(res_df, "./results/GSE165932_results.csv")
```
